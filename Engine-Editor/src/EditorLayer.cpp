...
namespace GameEngine
{
	...
	void EditorLayer::OnAttach()
	{
		...
		auto commandLineArgs = Application::Get().GetSpecificiation().CommandLineArgs;
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		if (m_ShowPhysicsColliders)
		{
			// Box Colliders
			{
				...
				view.each([](auto entity, auto& tc, auto& collider)
					{
	...
	{
		std::string filepath = FileDialogs::OpenFile("Game Scene (*.engine)\0*.engine\0");
		if (!filepath.empty())
		{
			OpenScene(filepath);
		}
	...
	{
		GE_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
