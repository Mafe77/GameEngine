...
namespace GameEngine
{
	...
	MonoAssembly* LoadCSharpAssembly(const std::string& assemblyPath)
	{
		...
		{
			GE_CORE_ERROR("Failed to read assembly file: {0}", assemblyPath);
			return nullptr;
		}

		MonoImageOpenStatus status;
		MonoImage* image = mono_image_open_from_data(fileData, fileSize, 1, &status);
		if (status != MONO_IMAGE_OK)
		{
			const char* errorMessage = mono_image_strerror(status);
			GE_CORE_ERROR("Mono image load failed for {0}: {1}", assemblyPath, errorMessage);
			delete[] fileData;
			return nullptr;
		}
		...
		if (status != MONO_IMAGE_OK)
		{
			GE_CORE_ERROR("Failed to load assembly from image {0}: {1}", assemblyPath, mono_image_strerror(status));
		}
	...
	void ScriptEngine::InitMono()
	{
		...
		MonoClass* monoClass = mono_class_from_name(assemblyImage, "GameEngine", "Main");
