...
namespace GameEngine
{
	...
	namespace Utils
	{
		...
		static MonoAssembly* LoadMonoAssembly(const std::filesystem::path& assemblyPath)
		{
			...
			if (status != MONO_IMAGE_OK)
			{
				...
				GE_CORE_ERROR("Mono image load failed for  {0}", errorMessage);
				delete[] fileData;
				return nullptr;
			}
	...
	void ScriptEngine::Init()
	{
		...
		s_Data->EntityClass = ScriptClass("GameEngine", "Entity");
	...
	void ScriptEngine::OnUpdateEntity(Entity entity, Timestep ts)
	{
		...
		GE_CORE_ASSERT(s_Data->EntityInstances.find(entityUUID) != s_Data->EntityInstances.end());
	...
	void ScriptEngine::LoadAssemblyClasses(MonoAssembly* assembly)
	{
		...
		const MonoTableInfo* typeDefinitionsTable = mono_image_get_table_info(image, MONO_TABLE_TYPEDEF);
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(image, "GameEngine", "Entity");
	...
	{
		return ScriptEngine::InstantianteClass(m_MonoClass);
	}
